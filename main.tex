...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Proyecciones de la celdilla unidad del zircón a lo largo de los ejes cristalográficos principales, mostrando la disposición de los átomos de Zr (verde), Si (azul) y O (gris).}
  \label{fig:zircon_uc}
\end{figure}

\vspace{0.5cm}

La uniformidad de las distancias se cuantifica mediante el índice de distorsión de longitud de enlace (BLD, del inglés \textit{Bond Length Distortion}), definido como:

\begin{equation}
  \text{BLD} = \frac{1}{n} \sum_{i=1}^{n} \frac{|d_i - \bar{d}|}{\bar{d}}
  \label{eq:bld}
\end{equation}

donde \(n\) es el número de enlaces, \(d_i\) la distancia del enlace \(i\), y \(\bar{d}\) la distancia promedio. Un valor de BLD = 0 indica un poliedro geométricamente ideal, mientras que valores mayores reflejan mayor irregularidad en las distancias de enlace.

\vspace{0.5cm}

\begin{minipage}[c]{0.75\textwidth} % Columna de TEXTO (Izquierda)
  \setlength{\parskip}{1em} % Mantiene espacio entre párrafos
  El tetraedro \ce{SiO4} presenta cuatro enlaces equivalentes con varianza angular de \SI{97.71}{\degree\squared} y alargamiento cuadrático de 1.0238, indicando compresión axial. Volumen: \SI{2.13}{\cubic\angstrom} (d = \SI{1.626}{\angstrom}, BLD = 0).
...
\begin{minipage}[c]{0.20\textwidth} % Columna de IMAGEN (Derecha)
  ...
  \begin{figure}[H]
    ...
    \caption{Tetraedro \ce{SiO4} del zircón }
    \label{fig:zircon_td}
  \end{figure}
\end{minipage}
...
\begin{minipage}[c]{0.70\textwidth} % Columna de IMAGEN (Derecha)
  \setlength{\parskip}{1em} % Mantiene espacio entre párrafos
  El poliedro \ce{ZrO8} exhibe geometría de bisdisfenoide con bimodalidad en distancias (4 cortas + 4 largas), volumen de \SI{22.13}{\cubic\angstrom} y coordinación efectiva de 7.45.
  (Zr--O: \SI{2.13}{\angstrom}--\SI{2.27}{\angstrom}, BLD = 0.0318)
\end{minipage}
\vspace{0.5cm}
...
\begin{table}[H]
  ...
  \begin{tabular}{l c c}
    ...
    Volumen (\si{\cubic\angstrom})          & 2.13      & 22.13     \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Proyecciones de la celdilla unidad del xenótimo a lo largo de los ejes cristalográficos principales. Código de colores: Y (verde claro), P (naranja), O (gris).}
...
\begin{minipage}[c]{0.70\textwidth}
  \setlength{\parskip}{1em} % Mantiene espacio entre párrafos
  Isoestructural con el zircón, el \ce{PO4} es 13.6\% más pequeño que el \ce{SiO4} debido a la mayor carga del \ce{P^{5+}}. Volumen: \SI{1.84}{\cubic\angstrom}.
\end{minipage}% <--- ¡IMPORTANTE: No borres este símbolo de porcentaje!
\hfill
\begin{minipage}[c]{0.25\textwidth} % Columna de IMAGEN (Derecha)
  ...
  \begin{figure}[H]
    ...
    \captionsetup{justification=centering}
    \caption{Tetraedro \ce{PO4} (d = \SI{1.533}{\angstrom}).}
...
\begin{minipage}[c]{0.65\textwidth}
  \setlength{\parskip}{1em} % Mantiene espacio entre párrafos
  El \ce{YO8} presenta la mitad de distorsión que el \ce{ZrO8} (BLD \(\sim\)50\% menor), consecuencia del mayor radio iónico del \ce{Y^{3+}}. Volumen: \SI{25.56}{\cubic\angstrom}.
  (Y--O: \SI{2.31}{\angstrom}--\SI{2.38}{\angstrom}, BLD = 0.0149)
\end{minipage}
...
\begin{minipage}[c]{0.70\textwidth} % Columna de TEXTO
  ...
  La simetría monoclínica genera bimodalidad en las distancias P--O, a diferencia de los tetraedros perfectos del zircón y xenótimo.
\end{minipage}
\hfill
\begin{minipage}[c]{0.25\textwidth} % Columna de IMAGEN (Opcional)
  ...
  \begin{figure}[H]
    ...
    \captionsetup{justification=centering}
    \caption{Tetraedro \ce{PO4} distorsionado (simetría \(C2/c\)).}
...
\begin{minipage}[c]{0.65\textwidth} % Texto a todo lo ancho
  \setlength{\parskip}{1em}
  Poliedro \ce{YO8} con 6 oxígenos de fosfato y 2 de agua estructural, característico de la fase hidratada.
\end{minipage}
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Proyecciones de la celdilla unidad de la monacita. Código de colores: Ce (amarillo), P (naranja), O (rosa).}
...
\begin{minipage}[c]{0.70\textwidth} % Columna de TEXTO (Izquierda)
  \setlength{\parskip}{1em}
  Altamente regular con elongación cuadrática de 1.0004 y coordinación efectiva de 3.99. Volumen: \SI{1.95}{\cubic\angstrom}.
\end{minipage}%
\hfill
\begin{minipage}[c]{0.25\textwidth} % Columna de IMAGEN (Derecha)
  \centering
  \begin{figure}[H]
    ...
    \captionsetup{justification=centering}
    \caption{Tetraedro \ce{PO4} regular (d = \SI{1.56}{\angstrom}, BLD = 0.0061).}
...
\begin{minipage}[c]{0.25\textwidth} % Columna de IMAGEN (Izquierda)
  ...
  \begin{figure}[H]
    ...
    \captionsetup{justification=centering}
    \caption{Eneacedro \ce{CeO9} (Ce--O: \SI{2.26}{\angstrom}--\SI{2.75}{\angstrom}, BLD = 0.056).}
...
\begin{minipage}[c]{0.70\textwidth} % Columna de TEXTO (Derecha)
  \setlength{\parskip}{1em}
  Poliedro de 9 vértices con distorsión significativa; coordinación efectiva de 6.76 indica interacción preferente con los oxígenos más cercanos. Volumen: \SI{31.85}{\cubic\angstrom}.
\end{minipage}
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Proyecciones de la celdilla unidad del rhabdofano. Código de colores: Ce (amarillo), P (naranja), O (rosa), \ce{H2O} (azul).}
...
\begin{minipage}[c]{0.70\textwidth} % Columna de TEXTO
  \setlength{\parskip}{1em}
  Alta regularidad consistente con la simetría hexagonal.
\end{minipage}
\hfill
\begin{minipage}[c]{0.25\textwidth}
  \begin{figure}[H]
    ...
    \captionsetup{justification=centering}
    \caption{Tetraedro \ce{PO4} simétrico (d = \SI{1.54}{\angstrom}, \(P6_222\)).}
...
\begin{minipage}[c]{0.3\textwidth} % Columna de IMAGEN (Izquierda)
  ...
  \begin{figure}[H]
    ...
    \captionsetup{justification=centering}
    \caption{Antiprisma \ce{CeO8} (d = \SI{2.53}{\angstrom}, BLD = 0.0631).}
...
\begin{minipage}[c]{0.65\textwidth} % Texto a todo lo ancho
  \setlength{\parskip}{1em}
  Geometría de antiprisma cuadrado distorsionado con dos grupos de 4 enlaces (\SI{2.33}{\angstrom} y \SI{2.64}{\angstrom}). Coordinación efectiva: 6.64. Volumen: \SI{25.95}{\cubic\angstrom} (el mayor de las estructuras estudiadas), consistente con los canales zeolíticos.
\end{minipage}
