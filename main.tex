...
% --- INICIO: MODIFICACIÓN PARA "TABLA" (Requisito del Usuario) ---
\usepackage{caption} 
\captionsetup[table]{name=Tabla}  

% --- FIN: MODIFICACIÓN PARA "TABLA" ---
...
\titlespacing*{\chapter}
  {0pt}{0pt}{1.5\baselineskip} % Sin espacio antes (0pt), sin espacio después (0pt), pero dejamos un espacio de una línea y media antes del texto para legibilidad.
...
\usepackage[
    backend=biber,
    style=apa,   % <--- ESTO pone los números [1], [2]...
    sorting=nyt,     % Ordena por Autor (como APA)
    language=spanish,
    citestyle= numeric  % <--- ESTO pone los números [1], [2]...
]{biblatex}

% 1. Quitar la "sangría rara" (margen izquierdo a 0)
\setlength{\bibhang}{0pt}

% 2. Añadir espacio entre cada cita (para que no se vean pegadas)
\setlength{\bibitemsep}{1.5\baselineskip}

% 1. Volvemos al estilo APA original (para que el texto se vea bien)
\usepackage[style=apa, backend=biber]{biblatex}

% --- INICIO DEL TRUCO DE MAGIA ---

% 2. Forzamos a LaTeX a numerar las referencias aunque sea APA
\ExecuteBibliographyOptions{labelnumber}

% 3. Redefinimos cómo se dibuja la lista para que aparezca el [1]
...
\defbibenvironment{bibliography}
  {\list
     ...
     {\setlength{\labelwidth}{\labelnumberwidth}%
      ...
      \setlength{\itemsep}{1.5\baselineskip} % Aquí mantenemos tu espacio de 1.5
...
\addbibresource{Bibliografia.bib} % <--- AJUSTAR NOMBRE DEL ARCHIVO
...
% NOTA: Estas páginas preliminares se excluyen del Índice de Contenidos a menos que se añadan manualmente.
...
\chapter{Resultados y Discusión}
 ...
 \section{Xenótimo (\ce{LnPO4}, Ln = Y, Tb-Lu)}
...
\section{Monacita (\ce{LnPO4},Ln = La-Gd)}
...
\begin{minipage}[c]{0.25\textwidth} % Columna de IMAGEN (Derecha)
    ...
    \begin{figure}[H]
        ...
        \label{fig:zircon_td} % Puedes cambiar la etiqueta si quieres
    \end{figure}
\end{minipage}
...
\vspace{0.5cm}
 ...
 \section{Churchita (\ce{LnPO4* nH2O}, Ln= Y, Tb-Lu)}
   Los parámetros cristalográficos para la representación de la Churchita \ce{YPO4*2H2O} se obtienen de ``Crystallography Open Database'' \footnote{\url{https://www.mindat.org/min-1047.html}} \cite{Kohlmann_na1039} .
...
\end{minipage}
\hfill
% NOTA: Si no tienes una imagen específica del tetraedro solo (tipo CHURCHITA_Td.png),
% puedes quitar este minipage de la derecha y dejar el texto ocupando todo el ancho.
...
\vspace{0.5cm}
 ...
 \section{Rhabdofano (\ce{LnPO4* nH2O}, Ln= Y, Tb-Lu)}
...
\end{minipage}
\hfill
% Si tienes una imagen del tetraedro solo, descomenta lo siguiente:
...
\begin{minipage}[c]{1.0\textwidth} % Texto a todo lo ancho
   \setlength{\parskip}{1em}
   Por otro lado, el catión cerio (\ce{Ce^{3+}}) se encuentra coordinado por ocho átomos de oxígeno pertenecientes a los grupos fosfato, conformando un poliedro \ce{CeO8} con geometría de antiprisma cuadrado distorsionado. A diferencia del entorno del fósforo, este sitio catiónico presenta una fuerte distorsión, evidenciada por un índice de 0.0631.
   
   El análisis de las distancias de enlace revela dos conjuntos claramente diferenciados: cuatro enlaces Ce--O más cortos de \SI{2.330}{\angstrom} y cuatro enlaces significativamente más largos de \SI{2.644}{\angstrom}. Esta disparidad resulta en una distancia media de enlace de \SI{2.487}{\angstrom} y un número de coordinación efectivo de 6.64, lo que sugiere que, aunque geométricamente hay 8 oxígenos vecinos, la interacción es mucho más fuerte con los cuatro más cercanos. El volumen del poliedro es de \SI{25.95}{\cubic\angstrom}, siendo el más voluminoso de las estructuras estudiadas, consistente con la naturaleza abierta de la estructura del Rhabdofano que aloja canales zeolíticos.
\end{minipage}
