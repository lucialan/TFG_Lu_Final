...
\begin{figure}[H]
  ...
  \caption{Example of radius dependence of the rare earth phosphates structures (from Chong et al., 2024 \cite{chong_synthesis_2024}).}
...
\begin{figure}[H]
  ...
  \caption{Schematic representation of lanthanide orthophosphates as matrices for nuclear waste immobilization.\cite{rafiuddin_review_2022}}
...
\begin{figure}[H]
  ...
  \caption{As demonstrated by the microscopy in Garrido Hernández et al.\cite{garrido_hernandez_photoluminescence_2016}, the hydrated monoclinic phase crystallizes as high-aspect-ratio nanorods. This specific morphology is identified by Chong et al.\cite{chong_synthesis_2024} as ideal for biomedical probes, unlike the granular shape of the tetragonal phase.}
...
El zircón es un prototipo estructura y referencia de estabilidad .Los parámetros cristalográficos del Zircón \ce{ZrSiO4} se obtienen de ``Materials Data on \ce{ZrSiO4} by Materials Project''\footnote{\url{https://next-gen.materialsproject.org/materials/mp-4820}} \cite{zircon_data_2020}. La estructura del Zircón, se caracteriza por un sistema cristalino tetragonal con grupo espacial \(I4_1/amd\) (No. 141) es de alta simetría ideal para cationes pequeños.

En la notación Hermann-Mauguin I indica un átomo centrado en la celda, 4\(_1\) indica un eje de \(90^\circ\) de rotación con una traslación de 1/4 de la altura de la celda unitaria, \(a\) indica un plano de simetría perpendicular al eje principal, \(m\) indica un plano de simetría y \(d\) indica un plano de simetría diagonal. El grupo puntual \(4/mmm\), \(4/m\) indica un eje de rotación de \(90^\circ\) con un plano de simetría horizontal perpendicular al eje principal \(c\). La segunda \(m\) indica  un plano de simetría vertical al eje principal pasando por los ejes \(a\) y \(b\). La tercera \(m\) indica un plano de simetría vertical al eje principal pasando por las diagonales del plano \(ab\). Con \(Z=4\) unidades de fórmula por celda unitaria.
...
\begin{minipage}[c]{0.70\textwidth} % Columna de TEXTO (Izquierda)
  ...
  Sin embargo, el poliedro no es un tetraedro geométrico ideal. El análisis de los ángulos de enlace revela una varianza angular de (\SI{97.71}{\degree})\(^2\) y un alargamiento cuadrático de 1.0238, valores que indican una compresión significativa del poliedro a lo largo del eje \(c\) (ver Figura \ref{fig:Zircón_big_a} El volumen calculado para este poliedro es de \SI{2.13}{\cubic\angstrom}.
...
La proyección a lo largo del eje \(b\) muestra una disposición similar de los poliedros \ce{ZrO8} y \ce{SiO4}, con una orientación diferente que resalta la simetría tetragonal de la estructura del Zircón.
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.45\linewidth}
    ...
    \caption{Proyección a lo largo del eje \(b\).}
  \end{subfigure}
  \caption{Representaciones de los poliedros y esqueletos del Xenótimo a lo largo del eje \(b\).}
...
En condiciones de síntesis a baja temperatura (precipitación), los HREE no forman directamente la fase anhidra (Xenótimo), sino una fase metaestable hidratada: la Churchita \cite{chong_synthesis_2024}. La incorporación de moléculas de agua en la red impide la simetría tetragonal, reduciendo el sistema a monoclínico (\(C2/c\)). Su estructura es laminar debido a la presencia de moléculas de agua en su red cristalina. Es la fase precursora de la xenotima, estable solo a bajas temperaturas (<200-300 °C). \cite{rafiuddin_review_2022,rafiuddin_structural_2022}.
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.45\linewidth}
    ...
    \caption{Proyección a lo largo del eje \(b\).}
  \end{subfigure}
  \caption{Representaciones de los poliedros y esqueletos del Churchita a lo largo del eje \(b\).}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.45\linewidth}
    ...
    \caption{Proyección a lo largo del eje \(b\).}
  \end{subfigure}
  \caption{Representaciones de los poliedros y esqueletos del Monacita a lo largo del eje \(b\).}
...
El Rhabdofano, análogamente al par Xenótimo-Churchita, al calentarse (500–900 °C) colapsa irreversiblemente a estructura monacita \cite{chong_synthesis_2024,shelyug_thermodynamics_2018}. Sin embargo, a diferencia de la estructura laminar de la Churchita, el Rhabdofano adopta una arquitectura hexagonal (\(P6_222\)) con canales zeolíticos abiertos en los cuales se alojan moléculas de agua estructurales, lo que permite variabilidad en su hidratación \cite{enikeeva_structure_2023,rafiuddin_review_2022}. . El Rhabdofano es un fosfato hidratado de tierras raras (Ln = Y, Tb-Lu) ligeras de mayor radio iónico.
