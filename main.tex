...
\begin{table}[H]
  \centering
  \caption{Sistemas cristalinos y sus parámetros de celda unitaria: restricciones geométricas que definen cada uno de los siete sistemas cristalográficos fundamentales.}
...
\begin{figure}[H]
  ...
  \caption{Representación de las 14 redes de Bravais: clasificación completa de las posibles disposiciones periódicas de puntos en el espacio tridimensional, agrupadas según los siete sistemas cristalinos y diferenciadas por sus tipos de centrado (P: primitivo, I: centrado en el cuerpo, F: centrado en las caras, C: centrado en la base).}
...
\begin{figure}[H]
  ...
  \caption{Dependencia estructural de los ortofosfatos de tierras raras con el radio iónico del lantánido: transición entre las fases Monacita (LREE, radio grande) y Xenótimo (HREE, radio pequeño), mostrando la existencia de un radio crítico de transición morfotrópica en la región Gd-Tb (adaptado de Chong et al., 2024 \cite{chong_synthesis_2024}).}
...
\begin{figure}[H]
  ...
  \caption{Representación esquemática de los ortofosfatos de lantánidos como matrices cerámicas para la inmovilización de residuos nucleares de alta actividad: incorporación de actínidos en las posiciones catiónicas de las estructuras tipo Monacita y Xenótimo, aprovechando su elevada resistencia a la radiación y baja solubilidad a largo plazo (adaptado de Rafiuddin et al., 2022 \cite{rafiuddin_review_2022}).}
...
\begin{figure}[H]
  ...
  \caption{Morfología de nanovarillas de alta relación de aspecto característica de las fases hidratadas monoclínicas de ortofosfatos de lantánidos: microscopía electrónica mostrando la formación de estructuras unidimensionales con potencial aplicación como sondas biomédicas, en contraste con la morfología granular de las fases tetragonales anhidras (de Garrido Hernández et al., 2016 \cite{garrido_hernandez_photoluminescence_2016}; Chong et al., 2024 \cite{chong_synthesis_2024}).}
...
\begin{table}[H]
  \centering
  \caption{Parámetros estructurales del Zircón: coordenadas atómicas fraccionarias, factores de ocupación, parámetros de desplazamiento térmico isotrópico (\(U\)), posiciones de Wyckoff y simetría local de los sitios cristalográficos en la estructura tetragonal \(I4_1/amd\).}
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Proyecciones de la celda unitaria del Zircón a lo largo de los ejes cristalográficos principales (\(a\), \(b\) y \(c\)), mostrando la disposición espacial de los átomos de Zr (verde), Si (azul) y O (rojo) en la red tetragonal \(I4_1/amd\) con \(Z=4\).}
  \label{fig:zircon_uc}
\end{figure}
...
\begin{minipage}[c]{0.20\textwidth} % Columna de IMAGEN (Derecha)
  ...
  \begin{figure}[H]
    ...
    \caption{Tetraedro \ce{SiO4} aislado del Zircón: poliedro de coordinación del silicio mostrando cuatro enlaces Si--O equivalentes de \SI{1.626}{\angstrom}, aunque con compresión apreciable a lo largo del eje \(c\) (varianza angular \SI{97.71}{\degree\squared}).}
    \label{fig:zircon_td}
  \end{figure}
\end{minipage}
...
\begin{minipage}[c]{0.25\textwidth}
  ...
  \begin{figure}[H]
    ...
    \captionsetup{justification=centering}
    \caption{Dodecaedro triangular (bisdisfenoide) \ce{ZrO8}: poliedro de coordinación del circonio con dos conjuntos cristalográficamente distintos de distancias Zr--O (4 enlaces cortos de \SI{2.13}{\angstrom} y 4 largos de \SI{2.27}{\angstrom}), resultando en un índice de distorsión BLD = 0.0318.}
...
\begin{minipage}[c]{0.40\textwidth} % Columna de IMAGEN (Derecha)
  ...
  \begin{figure}[H]
    ...
    \caption{Conectividad poliédrica en la estructura del Zircón: red tridimensional formada por tetraedros \ce{SiO4} aislados (amarillo) unidos a dodecaedros \ce{ZrO8} (verde) exclusivamente mediante vértices compartidos, generando cadenas alternas paralelas al eje \(c\) sin formación de aristas ni caras comunes entre poliedros.}
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Proyección poliédrica del Zircón a lo largo del eje \(a\): distribución de tetraedros \ce{SiO4} (amarillo) y dodecaedros \ce{ZrO8} (azul) en los planos (100) y (200), evidenciando el desplazamiento de medio parámetro \(a\) característico del plano de deslizamiento \(a\) del grupo espacial \(I4_1/amd\).}
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Representación en modo esqueleto (wireframe) del Zircón a lo largo del eje \(a\): disposición atómica en los planos (100) y (200) mostrando la conectividad de los átomos que conforman los poliedros \ce{ZrO8} y \ce{SiO4}, resaltando la simetría tetragonal de la estructura.}
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Proyección poliédrica del Zircón a lo largo del eje \(c\): vista a lo largo del eje tetragonal revelando la disposición ordenada de dodecaedros \ce{ZrO8} (azul) y tetraedros \ce{SiO4} (amarillo) en los planos (001) y (002), con los poliedros compartiendo vértices formando una red tridimensional de alta simetría.}
...
\begin{table}[H]
  \centering
  \caption{Parámetros de distorsión poliédrica en la estructura del Zircón: comparación cuantitativa entre los poliedros de coordinación \ce{SiO4} (tetraédrico, perfectamente simétrico en longitudes de enlace) y \ce{ZrO8} (dodecaédrico, con distorsión bimodal).}
...
%! ══════════════════════════════════════════════════════════════════════════
%! RECOMENDACIONES DEL README.md PARA MEJORAR ESTA SECCIÓN:
%! ══════════════════════════════════════════════════════════════════════════
%!
%! ESTRATEGIA: Cambiar de enfoque DESCRIPTIVO a COMPARATIVO
%! ─────────────────────────────────────────────────────────────────────────
%! En lugar de repetir toda la descripción estructural como en el Zircón,
%! enfatizar las DIFERENCIAS cuantitativas derivadas de la sustitución
%! heterovalente acoplada: Zr⁴⁺/Si⁴⁺ → Y³⁺/P⁵⁺
%!
%! PÁRRAFO INTRODUCTORIO SUGERIDO:
%! ─────────────────────────────────────────────────────────────────────────
%! "El Xenótimo cristaliza en el mismo grupo espacial que el Zircón (I4₁/amd),
%! constituyendo un ejemplo de ISOESTRUCTURALISMO derivado de la sustitución
%! heterovalente acoplada Zr⁴⁺ + Si⁴⁺ → Y³⁺ + P⁵⁺. Esta sustitución mantiene
%! la neutralidad de carga pero introduce diferencias significativas en las
%! distancias de enlace y la geometría poliédrica, consecuencia directa de
%! los distintos radios iónicos: r(Y³⁺) = 1.019 Å vs. r(Zr⁴⁺) = 0.84 Å 
%! en coordinación 8."
%!
%! TABLA COMPARATIVA (elemento central):
%! ─────────────────────────────────────────────────────────────────────────
%! En lugar de repetir tablas individuales, crear una TABLA COMPARATIVA
%! Zircón vs. Xenótimo que destaque:
%! - Parámetros de celda (a, b, c, V) con % de cambio
%! - Tetraedro TO₄: distancias T-O, varianza angular, volumen
%! - Dodecaedro MO₈: distancias M-O cortas/largas, índice BLD, volumen
%! - Columna con Δ (%) para cuantificar las diferencias
%!
%! TEXTO ANALÍTICO (no descriptivo):
%! ─────────────────────────────────────────────────────────────────────────
%! Interpretar los datos en lugar de repetir descripciones:
%! • Expansión anisotrópica: +4.1% en 'a', +0.5% en 'c' → refleja 
%!   orientación de dodecaedros en el plano ab
%! • Contracción del tetraedro: PO₄ más pequeño que SiO₄ (-13.6% volumen)
%!   por mayor carga del P⁵⁺, pero con mayor varianza angular
%! • Regularización del dodecaedro: BLD del YO₈ es ~50% del ZrO₈
%!
%! FIGURAS: Selección estratégica
%! ─────────────────────────────────────────────────────────────────────────
%! ✓ Proyecciones a, b, c: SÍ (necesarias, pero con caption comparativo)
%! ⚠ Tetraedro/dodecaedro individuales: OPCIONAL (solo si muestran diferencia visual)
%! ✗ Reducir proyecciones múltiples de poliedros (incluir solo UNA representativa)
%! ✓ NUEVA figura comparativa lado a lado: Zircón vs. Xenótimo [001]
%!
%! CIERRE DE SECCIÓN: Conexión con aplicaciones
%! ─────────────────────────────────────────────────────────────────────────
%! Conectar diferencias estructurales con propiedades físicas:
%! "La mayor regularidad del poliedro YO₈, combinada con la flexibilidad
%! del sitio para acomodar otros lantánidos pesados (Tb-Lu), fundamenta
%! su uso como matriz para inmovilización de actínidos en residuos nucleares.
%! La contracción lantánida permite modulación sistemática de parámetros
%! de celda a lo largo de la serie LnPO₄."
%!
%! ══════════════════════════════════════════════════════════════════════════
...
\begin{table}[H]
  \centering
  \caption{Parámetros estructurales del Xenótimo: coordenadas atómicas fraccionarias, factores de ocupación, parámetros de desplazamiento térmico isotrópico (\(U\)), posiciones de Wyckoff y simetría local de los sitios cristalográficos en la estructura tetragonal \(I4_1/amd\), isoestructural con el Zircón.}
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Proyecciones de la celda unitaria del Xenótimo a lo largo de los ejes cristalográficos principales (\(a\), \(b\) y \(c\)): estructura tetragonal \(I4_1/amd\) isoestructural con el Zircón, mostrando la disposición de átomos de Y (verde), P (violeta) y O (rojo), con \(Z=4\) unidades de \ce{YPO4} por celda.}
...
\begin{minipage}[c]{0.25\textwidth} % Columna de IMAGEN (Derecha)
  ...
  \begin{figure}[H]
    ...
    \captionsetup{justification=centering}
    \caption{Tetraedro \ce{PO4} del Xenótimo: poliedro de coordinación del fósforo con cuatro enlaces P--O idénticos de \SI{1.533}{\angstrom}, presentando menor distorsión angular (\SI{21.48}{\degree\squared}) y menor volumen (\SI{1.84}{\cubic\angstrom}) que el tetraedro \ce{SiO4} del Zircón, consistente con el mayor estado de oxidación del \ce{P^{5+}}.}
...
\begin{minipage}[C]{0.3\textwidth} % Columna de TEXTO (Izquierda)
  ...
  \begin{figure}[H]
    ...
    \caption{Dodecaedro \ce{YO8} del Xenótimo: poliedro de coordinación del itrio con dos conjuntos de distancias Y--O (4 enlaces cortos de \SI{2.311}{\angstrom} y 4 largos de \SI{2.391}{\angstrom}), mostrando menor distorsión (BLD = 0.0169) y mayor volumen (\SI{23.30}{\cubic\angstrom}) que el poliedro \ce{ZrO8} del Zircón debido al mayor radio iónico del \ce{Y^{3+}}.}
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Proyección poliédrica y en modo esqueleto del Xenótimo a lo largo del eje \(a\): distribución de dodecaedros \ce{YO8} (verde) y tetraedros \ce{PO4} (violeta) mostrando la red tridimensional característica de la estructura tipo Xenótimo/Zircón, con poliedros conectados exclusivamente mediante vértices compartidos.}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.45\linewidth}
    ...
    \caption{Proyección a lo largo del eje \(b\).}
  \end{subfigure}
  \caption{Proyección poliédrica y en modo esqueleto del Xenótimo a lo largo del eje \(b\): vista complementaria de la estructura tetragonal mostrando la disposición equivalente de poliedros respecto al eje \(a\) (rotación de 90°), confirmando la simetría \(a = b\) del sistema tetragonal.}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.45\linewidth}
    ...
    \caption{Proyección del esqueleto}
  \end{subfigure}
  \caption{Proyección poliédrica y en modo esqueleto del Xenótimo a lo largo del eje \(c\): vista perpendicular al eje tetragonal evidenciando la disposición ordenada de dodecaedros \ce{YO8} y tetraedros \ce{PO4} en el plano basal, característica de la alta simetría del grupo espacial \(I4_1/amd\).}
...
\begin{figure}[H]
  ...
  \caption{Representación tridimensional en perspectiva de la estructura poliédrica del Xenótimo: visualización de la red completa de dodecaedros \ce{YO8} (verde) y tetraedros \ce{PO4} (violeta) interconectados, mostrando la arquitectura tridimensional característica de la fase tetragonal anhidra de ortofosfatos de lantánidos pesados.}
...
\begin{table}[H]
  \centering
  \caption{Parámetros estructurales de la Churchita: coordenadas atómicas fraccionarias, factores de ocupación, parámetros de desplazamiento térmico isotrópico (\(U\)), posiciones de Wyckoff y simetría local de los sitios cristalográficos en la estructura monoclínica \(C2/c\), incluyendo las posiciones del agua estructural (Ow).}
...
\begin{figure}[H]
  ...
  \begin{minipage}[c]{0.25\linewidth} % Ocupa el 45% del ancho
    ...
    \end{subfigure}
  \end{minipage}
  \caption{Proyecciones de la celda unitaria de la Churchita a lo largo de los ejes cristalográficos principales (\(a\), \(b\) y \(c\)): estructura monoclínica \(C2/c\) mostrando la disposición de átomos de Y (verde), P (violeta), O (rojo) y moléculas de agua estructural (celeste), con \(Z=4\) unidades de \ce{YPO4·2H2O} por celda.}
...
\begin{minipage}[c]{0.25\textwidth} % Columna de IMAGEN (Opcional)
  ...
  \begin{figure}[H]
    ...
    \captionsetup{justification=centering}
    \caption{Tetraedro \ce{PO4} de la Churchita: poliedro de coordinación del fósforo con ligera distorsión respecto al Xenótimo, presentando dos conjuntos de enlaces (P--O1: \SI{1.536}{\angstrom} y P--O2: \SI{1.551}{\angstrom}), con índice de distorsión BLD = 0.0048 y varianza angular de \SI{18.96}{\degree\squared}.}
...
\begin{minipage}[c]{0.3\textwidth} % Columna de IMAGEN (Izquierda)
  ...
  \begin{figure}[H]
    ...
    \captionsetup{justification=centering}
    \caption{Poliedro de coordinación \ce{YO6(H2O)2} de la Churchita: entorno octaédrico distorsionado del itrio con coordinación mixta que incluye oxígenos de grupos fosfato y moléculas de agua estructural, mostrando cuatro conjuntos de distancias Y--O diferenciadas (desde \SI{2.251}{\angstrom} hasta \SI{2.469}{\angstrom}), con índice de distorsión BLD = 0.0308.}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.45\linewidth}
    ...
    \caption{Proyección del esqueleto}
  \end{subfigure}
  \caption{Proyección poliédrica y en modo esqueleto de la Churchita a lo largo del eje \(a\): estructura laminar monoclínica característica de la fase hidratada, mostrando la disposición de poliedros \ce{YO6(H2O)2} (verde) y tetraedros \ce{PO4} (violeta), con moléculas de agua estructural integrando la esfera de coordinación del itrio.}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.45\linewidth}
    ...
    \caption{Proyección a lo largo del eje \(b\).}
  \end{subfigure}
  \caption{Proyección poliédrica y en modo esqueleto de la Churchita a lo largo del eje \(b\): vista perpendicular al eje binario monoclínico evidenciando la arquitectura laminar y la ruptura de la simetría tetragonal del Xenótimo debido a la incorporación de moléculas de agua estructural.}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.45\linewidth}
    ...
    \caption{Proyección del esqueleto}
  \end{subfigure}
  \caption{Proyección poliédrica y en modo esqueleto de la Churchita a lo largo del eje \(c\): disposición de poliedros en el plano perpendicular al eje monoclínico, mostrando la conectividad tridimensional alterada por la presencia de agua estructural respecto a la fase anhidra Xenótimo.}
...
\begin{figure}[H]
  ...
  \caption{Representación tridimensional en perspectiva de la estructura poliédrica de la Churchita: visualización completa de la arquitectura laminar monoclínica formada por poliedros \ce{YO6(H2O)2} (verde) y tetraedros \ce{PO4} (violeta), característica de la fase hidratada metaestable precursora del Xenótimo, estabilizada por enlaces de hidrógeno con las moléculas de agua estructural.}
...
\begin{table}[H]
  \centering
  \caption{Parámetros estructurales de la Monacita: coordenadas atómicas fraccionarias, factores de ocupación, parámetros de desplazamiento térmico isotrópico (\(U\)), posiciones de Wyckoff y simetría local de los sitios cristalográficos en la estructura monoclínica \(P2_1/n\), característica de lantánidos ligeros con coordinación 9.}
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Proyecciones de la celda unitaria de la Monacita a lo largo de los ejes cristalográficos principales (\(a\), \(b\) y \(c\)): estructura monoclínica \(P2_1/n\) mostrando la disposición de átomos de Ce (verde), P (violeta) y O (rojo), con \(Z=4\) unidades de \ce{CePO4} por celda y número de coordinación 9 para el lantánido.}
...
\begin{minipage}[c]{0.25\textwidth} % Columna de IMAGEN (Derecha)
  ...
  \begin{figure}[H]
    ...
    \captionsetup{justification=centering}
    \caption{Tetraedro \ce{PO4} de la Monacita: poliedro de coordinación del fósforo extremadamente regular con distancias P--O homogéneas (promedio \SI{1.56}{\angstrom}), bajo índice de distorsión BLD = 0.0061, elongación cuadrática de 1.0004 y número de coordinación efectivo de 3.99, actuando como unidad estructural rígida de la red.}
...
\begin{minipage}[c]{0.25\textwidth} % Columna de IMAGEN (Izquierda)
  ...
  \begin{figure}[H]
    ...
    \captionsetup{justification=centering}
    \caption{Eneacedro \ce{CeO9} de la Monacita: poliedro de coordinación del cerio con geometría de eneacedro significativamente distorsionado (BLD = 0.05599), mostrando amplia dispersión de distancias Ce--O (desde \SI{2.26}{\angstrom} hasta \SI{2.75}{\angstrom}), número de coordinación efectivo de 6.76 y volumen de \SI{31.85}{\cubic\angstrom}, característico de lantánidos ligeros de gran radio iónico.}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.45\linewidth}
    ...
    \caption{Proyección del esqueleto}
  \end{subfigure}
  \caption{Proyección poliédrica y en modo esqueleto de la Monacita a lo largo del eje \(a\): estructura monoclínica característica de lantánidos ligeros (LREE), mostrando la disposición de eneacedros \ce{CeO9} (verde) de alta distorsión y tetraedros \ce{PO4} (violeta) rígidos en una red tridimensional de menor simetría que el Xenótimo.}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.45\linewidth}
    ...
    \caption{Proyección a lo largo del eje \(b\).}
  \end{subfigure}
  \caption{Proyección poliédrica y en modo esqueleto de la Monacita a lo largo del eje \(b\): vista perpendicular al eje helicoidal \(2_1\) evidenciando la arquitectura tridimensional resultante de la transición morfotrópica inducida por el mayor radio iónico del \ce{Ce^{3+}} respecto a lantánidos pesados.}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.45\linewidth}
    ...
    \caption{Proyección del esqueleto}
  \end{subfigure}
  \caption{Proyección poliédrica y en modo esqueleto de la Monacita a lo largo del eje \(c\): disposición de poliedros en el plano perpendicular al eje monoclínico, mostrando la conectividad característica de la estructura tipo Monacita con número de coordinación 9 para el lantánido.}
...
\begin{figure}[H]
  ...
  \caption{Representación tridimensional en perspectiva de la estructura poliédrica de la Monacita: visualización completa de la red formada por eneacedros \ce{CeO9} (verde) altamente distorsionados y tetraedros \ce{PO4} (violeta) rígidos, característica de la fase anhidra estable de ortofosfatos de lantánidos ligeros (La-Gd) con grupo espacial monoclínico \(P2_1/n\).}
...
\begin{table}[H]
  \centering
  \caption{Parámetros estructurales del Rhabdofano: coordenadas atómicas, factores de ocupación, parámetros térmicos isotrópicos, posiciones de Wyckoff y simetría local para la estructura hexagonal \(P6_222\) de \ce{CePO4·0.67H2O}.}
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Proyecciones de la celda unitaria del Rhabdofano a lo largo de los ejes cristalográficos principales (\(a\), \(b\) y \(c\)): estructura hexagonal \(P6_222\) mostrando la disposición de átomos de Ce (verde), P (violeta), O (rojo) y moléculas de agua zeolítica en los canales (celeste), con \(Z=3\) unidades de \ce{CePO4·0.67H2O} por celda.}
...
\begin{minipage}[c]{0.25\textwidth}
  \begin{figure}[H]
    ...
    \captionsetup{justification=centering}
    \caption{Tetraedro \ce{PO4} del Rhabdofano: poliedro de coordinación del fósforo cristalográficamente perfecto con cuatro distancias P--O idénticas de \SI{1.558}{\angstrom}, índice de distorsión BLD = 0.0000, elongación cuadrática de 1.0000 y varianza angular despreciable (\SI{0.018}{\degree\squared}), superior en regularidad a las fases monoclínicas (Churchita, Monacita).}
...
\begin{minipage}[c]{0.3\textwidth} % Columna de IMAGEN (Izquierda)
  ...
  \begin{figure}[H]
    ...
    \captionsetup{justification=centering}
    \caption{Poliedro \ce{CeO8} del Rhabdofano: antiprisma cuadrado distorsionado con dos conjuntos de distancias Ce--O (4 enlaces cortos de \SI{2.330}{\angstrom} y 4 largos de \SI{2.644}{\angstrom}), elevado índice de distorsión BLD = 0.0631, número de coordinación efectivo de 6.64 y volumen máximo de \SI{25.95}{\cubic\angstrom}, característico de la arquitectura abierta con canales zeolíticos.}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.45\linewidth}
    ...
    \caption{Proyección del esqueleto}
  \end{subfigure}
  \caption{Proyección poliédrica y en modo esqueleto del Rhabdofano a lo largo del eje \(a\): estructura hexagonal con canales zeolíticos paralelos al eje \(c\), mostrando la disposición de poliedros \ce{CeO8} (verde) tipo antiprisma cuadrado y tetraedros \ce{PO4} (violeta), con moléculas de agua alojadas en los canales de la red.}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.45\linewidth}
    ...
    \caption{Proyección a lo largo del eje \(b\).}
  \end{subfigure}
  \caption{Proyección poliédrica y en modo esqueleto del Rhabdofano a lo largo del eje \(b\): vista complementaria de la estructura hexagonal evidenciando la simetría senaria y la arquitectura abierta característica, precursora metaestable de la fase anhidra Monacita tras deshidratación a 500-900 °C.}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.45\linewidth}
    ...
    \caption{Proyección del esqueleto}
  \end{subfigure}
  \caption{Proyección poliédrica y en modo esqueleto del Rhabdofano a lo largo del eje \(c\): vista perpendicular al eje senario revelando la disposición hexagonal de poliedros y los canales zeolíticos abiertos que alojan moléculas de agua con hidratación variable (\ce{LnPO4·nH2O}), diferenciándolo estructuralmente de la fase laminar Churchita.}
...
\begin{figure}[H]
  ...
  \caption{Representación tridimensional en perspectiva de la estructura poliédrica del Rhabdofano: visualización completa de la arquitectura hexagonal \(P6_222\) formada por poliedros \ce{CeO8} (verde) tipo antiprisma cuadrado y tetraedros \ce{PO4} (violeta), característica de la fase hidratada metaestable de ortofosfatos de lantánidos ligeros con canales zeolíticos de hidratación variable.}
